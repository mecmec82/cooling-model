...
def model(T, t, Q_edu, Kp, Ki, Kd, T_setpoint, integral, prev_error, ramp_up_time, ramp_down_time, prev_Q_cond, elapsed_time):
    error = T_setpoint - T
    dt = t[1] - t[0]
    integral += error * dt
...
Ki = st.sidebar.slider("PID Integral Gain (Ki)", 0.0, 100.0, 0.0)
Kd = st.sidebar.slider("PID Derivative Gain (Kd)", 0.0, 100.0, 0.0)
...
for t in range(len(time) - 1):
    dTdt, integral, prev_error, Q_cond, elapsed_time = model(
        ...
        ramp_up_time, ramp_down_time, prev_Q_cond, elapsed_time
    )
